...
```powershell
pnpm install
# .env.local에 API 키 채우기 (.env.example 참조)
...
5. **Environment Variables**: 필수 2개 + 선택 3개 (Production / Preview 모두 체크).
   - `MOLIT_API_KEY` — 국토부 실거래가 OpenAPI Decoding 키 (서버 전용)
   - `NEXT_PUBLIC_KAKAO_MAP_KEY` — 카카오 지도 JS 키 (도메인 화이트리스트로 보호)
   - (선택) `NEXT_PUBLIC_GA_ID` — Google Analytics 4 측정 ID (`G-XXXXXXXXXX`)
   - (선택) `NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION` — 구글 서치 콘솔 인증 토큰
   - (선택) `NEXT_PUBLIC_NAVER_SITE_VERIFICATION` — 네이버 서치어드바이저 인증 토큰
...
도메인이 연결된 후 다음 두 곳에 사이트를 등록해야 검색에 노출됩니다.

**Google Search Console**

1. <https://search.google.com/search-console> → 속성 추가 → URL 접두어 → `https://daerim.com`
2. "HTML 태그" 방식 선택 → `<meta name="google-site-verification" content="XXXX...">` 의
   `content` 값 복사
3. Vercel 환경변수에 `NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION=XXXX...` 추가 → Redeploy
4. 다시 Search Console에서 "확인" 클릭
5. 사이트맵 제출: `https://daerim.com/sitemap.xml`

**네이버 서치어드바이저**

1. <https://searchadvisor.naver.com/console> → 사이트 등록 → `https://daerim.com`
2. "HTML 태그" 방식 → `<meta name="naver-site-verification" content="XXXX...">` 의
   `content` 값 복사
3. Vercel 환경변수에 `NEXT_PUBLIC_NAVER_SITE_VERIFICATION=XXXX...` 추가 → Redeploy
4. "소유 확인"
5. 사이트맵 제출 + RSS 제출 (`https://daerim.com/feed.xml`)

### 5) 분석 (선택)

**Vercel Analytics / Speed Insights**: 별도 셋업 없이 자동 수집됩니다.
Vercel Dashboard의 Analytics 탭에서 페이지 뷰·Core Web Vitals 확인 가능.

**Google Analytics 4**: <https://analytics.google.com> → 속성 만들기 → 데이터 스트림(웹) →
측정 ID(`G-XXXXXXXXXX`) 발급. Vercel 환경변수에 `NEXT_PUBLIC_GA_ID` 추가 후 Redeploy.

## 어드민 (매물 CMS)

`/admin` 경로에서 매물을 직접 등록·수정·삭제·사진 업로드할 수 있습니다.
Supabase 셋업이 필요합니다 — 자세한 절차는 [`SUPABASE_SETUP.md`](./SUPABASE_SETUP.md).

## 로드맵

- [x] Sprint 1: 기초 셋업 (홈/About/Contact 골격, 디자인 토큰, 단지 데이터 시드)
- [x] Sprint 2: 시그니처 도구 (실거래가 추이 + 단지 비교 대시보드)
- [x] Sprint 3: 하이닉스 매칭 + 매물 + 시뮬레이터
- [x] Sprint 4: 블로그 콘텐츠 + SEO
- [x] Phase 2 (1단계): Supabase + 매물 어드민
- [ ] Phase 2 (2단계): 매물 알림 구독자
- [ ] Phase 2 (3단계): 실거래가 일별 누적 cron
- [ ] Phase 2 (4단계): 방문자 분석 대시보드
